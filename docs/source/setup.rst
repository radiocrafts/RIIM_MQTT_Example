...
We need to install a CoAP extension and MQTT extencion(Aedes) to Node RED and import our flow. Start by clicking the menu in the top right corner and select **Manage palette** . Click the **Install** tab and type "coap" in the search bar. Select to install **node-red-contrib-coap**. Node RED is now set up with support for CoAP.
...
Then the MQTT broker can be added in the same manner. Search up Aedes on the manage palette and install it.

.. image:: _static/images/NR_Install_Aedes_MQTT_broker.png
  :width: 400
  :alt: Install the node-red-contrib-aedes package

Now we need our flow. Open the menu again and select **Import**. Import the file located in ./NodeRed/MQTT_Example_NodeRed.json.
